	...
	export default {
		...
		methods: {
			...
			getOrder(id) {
				...
				this.id = id
				this.goodList = []
				this.$common.httpGet('/order_agent/order/getById/' + this.id).then((res) => {
					// console.log(res)
			...
			payway() {
				...
				if (this.Info.orderSn) {
					...
					this.$common.httpPost('order_agent/order/createOrder', parmas).then((res) => {
						console.log(res)
						if (Number(res.data.code) === 300200) {
							...
							uni.requestPayment({
								...
								success: (res) => {
									...
									if (this.Info.comboLevel) {
										...
										this.$common.httpPost('up_mall_rebate_agent/rebate/agentIntiveBuyCombo', BuyCombo).then((res) => {
			...
			cancelOrder() {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.$common.httpPost('order_agent/order/cancel/' + this.id, true).then((res) => {
								if (Number(res.data.code) === 300200) {
			...
			confirmProduct() {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.$common.httpPost('order_agent/order/updateOrderStatus/' + this.id, true).then((res) => {
								if (Number(res.data.code) === 300200) {
									this.$common.showToast1(res.data.message, 'none')
									// 设定3秒后刷新
									// this.getOrder(this.id)
