<template>
	<view class="product">
		...
		<view class="product-banner">
			...
			<swiper class="swiper" :indicator-dots="true" indicator-color="#000000" indicator-active-color="#5B8EDB" interval="2000"
			 duration="500" v-else>
		...
		<view class="popup spec" :class="specClass" @touchmove.stop.prevent="stopPrevent" @click="toggleSpec">
			...
			<view class="layer attr-content" @click.stop="stopPrevent">
				...
				<view class="con_count ">
					...
					<view class="item">
						<!-- <uni-number-box disabled="false" :min="1" @change="bindChange" :max="specDetail.stock" :value="num"></uni-number-box> -->
						<uni-number-box  :min="1" @change="bindChange" :max="specDetail.stock" :value="num"></uni-number-box>
					</view>
				</view>
		...
		<view class="product-bottom">
			<view class="" style="display: flex;">
				<view class="iconImg" @click="goUserInfo">
					<view class="iconfont icon-mine-gray" style="font-size: 48rpx; color: #5B8EDB;"></view>
				</view>
				<view class="iconImg" style="margin-left: 40rpx;" @click="goIndex">
					<view class="iconfont icon-yemian-copy-copy" style="font-size: 48rpx; color: #5B8EDB;"></view>
				</view>
			</view>

			<view class="iconImg" style="margin-left: 40rpx;" @tap="goshoppingCart">
				<view class="iconfont icon-gouwuche" style="font-size: 48rpx; color: #5B8EDB;"></view>
			</view>
...
<script>
	...
	export default {
		...
		onShow() {
			...
			this.getAstrict(this.optionInfo.spu)
		},
		methods: {
			// 获取商品限购数量
			getAstrict(e){
				this.$common.httpGet('order_agent/groupon/isBuyLimit?spuId=' + e, false).then(res => {
					console.log('获取商品限购数量',res)
			...
			getProductDetail(spId) {
				this.$common
					.httpGet('common_goods_agent/common/spuDetail', {
						spuId: spId,
						system: 'UPMALL'
					},false)
					.then(res => {
			...
			// 获取优惠价格
			getDiscounts(spId) {
				this.$common.httpGet('common_goods_agent/upMall/userPrice/spu/' + spId).then(res => {
					if (res.data.code === 200200) {
			...
			getStandardInfo() {
				...
				this.$common
					.httpGet('common_goods_agent/common/skuSpec', {
						spuId: this.spuId,
					...
					.then(res => {
						if (res.data.code === 200200) {
							...
							// this.$common.httpGet('common_goods/upMall/userPrice/sku/'+this.specDetail.skuid).then(res => {
			...
			// SKU用户售价
			getSkuPrice(skuid) {
				this.$common.httpGet('common_goods_agent/upMall/userPrice/sku/' + skuid,false,false).then(res => {
					if (res.data.code === 200200) {
			...
			// 获取商品背后的故事
			getSpuExtraInfo() {
				this.$common.getNoToken('common_goods_agent/upMall/uncalibrated/spuExtraInfo/' + this.spuId).then(res => {
					if (res.data.code === 200200) {
			...
			handelTmitation(){
				if(Number(this.astrict) === 0) {
					this.purchase()
					return
				}else if (Number(this.num) > Number(this.astrict)) {
					this.$common.showToast('超出限制购买数量');
					return
				}else {
					this.purchase()
					return
			...
			addShoppingCart: throttle.throttleFunc(function() {
				...
				// 	if (res) {
						// setTimeout(() => {
							this.$common.httpGet('common_goods_agent/common/shoppingCart/addition', {
