...
<script>
	...
	export default {
		...
		methods: {
			...
			getOrder() {
				this.goodList = []
				this.$common.httpGet('/order_agent/order/getOrderPages?pageNum=' + this.param.pageNum + '&pageSize=' + this.param.pageSize +
			...
			cancelOrder(item) {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							this.$common.httpPost('order_agent/order/cancel/' + item.orderId, true).then((res) => {
								console.log(res)
								this.param.pageNum = 1
								if (Number(res.data.code) === 300200) {
			...
			confirmProduct(item) {
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							// console.log('用户点击确定');
							this.$common.httpPost('order_agent/order/updateOrderStatus/' + item.orderId, true).then((res) => {
								console.log(res)
								this.param.pageNum = 1
								if (Number(res.data.code) === 300200) {
									this.$common.showToast1(res.data.message, 'none')
