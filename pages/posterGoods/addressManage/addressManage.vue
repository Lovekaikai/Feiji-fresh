...
<script>
	...
	export default {
		...
		methods: {
			...
			setDefault() {
				this.$common.httpGet('user_agent/userAddress/getPages?pageNum=' + this.pageNum).then((res) => {
			...
			selectAdress(item) {
				if (this.page === 'exchange') {
					this.$common.setStorage("Agentaddress2", item)
				} else {
					this.$common.setStorage("Agentaddress", item)
				}
			...
			bindClick(value, id) {
				...
				if (value.text === '删除') {
					uni.showModal({
						...
						success: (res) =>{
							if (res.confirm) {
								_this.$common.httpPost('user_agent/userAddress/deleteById?id=' + _this.addressid).then((
									res) => {
									if (res.data.code === 100200) {
										console.log(121)
										// _this.$common.showToast('已删除', 'none')
										let infoAddress = _this.$common.getStorage('Agentaddress')
					...
					uni.showModal({
						...
						success: (res) => {
							if (res.confirm) {
								_this.$common.httpPost('user_agent/userAddress/updateDefault?id=' + _this.addressid).then(
									(res) => {
										if (res.data.code === 100200) {
											// _this.$common.showToast('已删除', 'none')
											let infoAddress = this.$common.getStorage('Agentaddress')
											if (infoAddress.id === _this.addressid) {
												infoAddress.isDefault = 1
											}
											this.$common.setStorage('Agentaddress', infoAddress)
											_this.getAddresslist()
										}
									})
							} else if (res.cancel) {
