...
<script>
	...
	export default {
		...
		onShow() {
			this.address = this.$common.getStorage('Agentaddress');
		},
		onLoad(e) {
		...
		methods: {
			...
			// 检查用户是否有默认地址
			getDefault() {
				this.$common.httpGet('user_agent/userAddress/getDefault').then(res => {
					console.log(res);
					if (Number(res.data.code) === 100200) {
						// this.packageList = res.data.data;
						if (res.data.data) {
							this.address = res.data.data;
							this.$common.setStorage('Agentaddress', this.address);
						} else {
							this.address = this.$common.getStorage('Agentaddress') || {};
						}
					} else {
			...
			// 获取可用余额
			getBalance() {
				this.balance = this.$common.getStorage('agentInfo').balance
			},
			// 选择支付方式
			getpayWay(e) {
