	...
	<view class="shoppingCart">
		...
		<view class="shoppingCart-shop" v-if="cartList.length > 0">
			<view class="shoppingCart-shop-left">
				<view class="shop-img">
					<image src="../../../static/images/shop_big.png" mode=""></image>
				</view>
				...
				<view class="productItem" v-for="(proItem, pIdx) in cartList" :key="pIdx">
					...
					<view class="productItem-right" @tap="goToProduct(proItem.spuId)">
						...
						<view class="productItem-right-right">
							...
							<view class="opsNum">
								<view class="ops subtract" @tap.stop="calcValue(-1, pIdx,proItem.spuId)">-</view>
								<view class="proNum">{{ proItem.count }}</view>
<!-- 								<view class="ops plus" v-if="proItem.spuId === 270 || proItem.spuId === 292">+</view> -->
								<view class="ops plus" @tap.stop="calcValue(1, pIdx,proItem.spuId)">+</view>
							</view>
						</view>
					</view>
		...
		<view class="shoppingCart-footer" v-if="cartList.length > 0">
			...
			<view class="shoppingCart-footer-right" v-if="isShowPay">
				<view class="left">
					<view class="">
						...
						<text style="color: rgba(91, 142, 219, 1);font-size: 20rpx;font-weight: bold;">￥</text>
						<text style="color: rgba(91, 142, 219, 1);font-size: 36rpx;font-weight: bold;">{{ totalPrice }}</text>
					</view>
			...
			<view class="shoppingCart-footer-right" v-else>
				<view class="delete" @tap="delectProduct">删除</view>
			</view>
		</view>
		...
		<view class="shoppingCart-empty" v-if="isShowEmpty">
			<view class="content">
				<image src="https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1587384576224&di=de66d809eb08296fa53e03049da48f01&imgtype=0&src=http%3A%2F%2Fku.90sjimg.com%2Felement_origin_min_pic%2F01%2F39%2F08%2F88573ca8ba0c141.jpg"
				 mode="widthFix"></image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			calcValue(num, idx, goodsId) {
				...
				if (count === 1) {
					if (Number(num) === -1) {
						...
						// 				system: 'UPMALL'
						// 			}
						// 			this.$common.httpGet('common_goods/common/shoppingCart/removeSku',params).then(res => {
				...
				// 判断是同一个商品进行操作
				if (Number(this.spuId) === Number(goodsId)) {
					// 判断是否限购
					if (Number(this.astrict) === 0) {
						...
						this.$common.httpGet('common_goods_agent/common/shoppingCart/addition', requestObj, false).then(res => {
							// console.log(res)
						})
						this.countData()
						this.spuId = goodsId
						return
					} else if (Number(newCount) > Number(this.astrict)) {
						this.$common.showToast('商品限制购买' + this.astrict + '个')
						this.spuId = goodsId
						return
					} else {
						this.$set(this.cartList[idx], 'count', newCount)
						...
						requestObj4.increase = num === 1 ? true : false
						this.$common.httpGet('common_goods_agent/common/shoppingCart/addition', requestObj4, false).then(res => {
							// console.log(res)
						})
						this.countData()
						this.spuId = goodsId
					...
					// 获取商品可购买数量
					this.$common.httpGet('order_agent/groupon/isBuyLimit?spuId=' + goodsId, false).then(res => {
						if (Number(res.data.code) === 300200) {
							this.astrict = res.data.data
							if(Number(this.astrict) === 0) {
								this.$set(this.cartList[idx], 'count', newCount)
								let requestObj2 = {
									skuId,
								...
								requestObj2.increase = num === 1 ? true : false
								this.handleAddition(requestObj2)
								this.countData()
								this.spuId = goodsId
								return
							} else {
								if (Number(newCount) > Number(this.astrict)) {
									this.$common.showToast('商品限制购买' + this.astrict + '个')
									this.spuId = goodsId
									return
								} else {
									...
									requestObj3.increase = num === 1 ? true : false
									this.handleAddition(requestObj3)
									this.countData()
									this.spuId = goodsId
									return
								}
				...
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							...
							this.$common.httpGet('common_goods_agent/common/shoppingCart/removeSku', params).then(res => {
								if (res.data.code === 200200) {
									this.$common.showToast('删除成功')
			...
			handleAddition(e){
				console.log(e)
				this.$common.httpGet('common_goods_agent/common/shoppingCart/addition', e, false).then(res => {
					// console.log(res)
				})
			},
			// 获取购物车列表
			getCarList() {
				this.$common.httpGet('common_goods_agent/common/shoppingCart/cart', {
					system: 'UPMALL'
				}).then(res => {
					console.log(res);
					const {
...
<style lang="less" scoped>
		...
		.shoppingCart-footer {
			...
			.shoppingCart-footer-right {
				...
				.delete {
					...
					height: 62rpx;
					border-radius: 31rpx;
					border: 1px solid rgba(91, 142, 219, 1);
					color: rgba(91, 142, 219, 1);
	...
	.checkbox {
		...
		height: 28rpx;
		background: rgba(255, 255, 255, 1);
		border: 2rpx solid rgba(91, 142, 219, 1);
		border-radius: 50%;
		// display: flex;
		// align-items: center;
		...
		.on {
			...
			height: 18upx;
			border-radius: 50%;
			background-color: rgba(91, 142, 219, 1);
			position: absolute;
			top: 50%;
			left: 50%;
			transform: translate(-50%, -50%);
