<template>
	<view class="index">
		...
		<view class="content" v-show="!IsSkelttion">
			...
			<!-- 底部分类 -->
			<menu-nav :NavTopMuch='NavTopMuch' :classify-title="classifyTitle" :list="classifyGoods" :current-index="currentIndex"
			 @change="changeTitle" :classify-empty="classifyEmpty"></menu-nav>
					...
					if (res.data.code === 200200) {
						...
						for (let s of res.data.data) {
							arr.push({
								...
								spuId:s.spuId,
								skuName:s.skuName,
								initPrice:s.skuPrice
							})
						}
						this.classifyEmpty =arr.length > 0 ? false : true // 有数据则隐藏空 无则显示空
						this.classifyGoods=arr
					} else {
						this.classifyGoods = [];
					}
				});
			  ...
			  this.$common
			    ...
			    .then(res => {
			      if (Number(res.data.code) === 200200) {
			        // console.log(res) 
					this.classifyEmpty = res.data.data.list.length > 0 ? false : true // 有数据则隐藏空 无则显示空
			        //把数据更新到原来数组添加
			        this.classifyGoods = [...this.jointList, ...res.data.data.list];
			        // this.total= res.data.data.total  
			        // console.log("this.classifyGoods",this.classifyGoods)
			        callBack && callBack();
			      } else {
					this.classifyGoods = [];
